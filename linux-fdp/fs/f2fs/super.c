	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_discard:
			...
			if (!f2fs_hw_support_discard(sbi)) {
				printk("@@@@@@@@@@ NO SUPPORT DISCARD\n");
				f2fs_warn(sbi, "device does not support discard");
				break;
			}
			printk("@@@@@@@@@@ SUPPORT DISCARD\n");
			set_opt(sbi, DISCARD);
			break;
		case Opt_nodiscard:
		...
		case Opt_discard_unit:
			...
			if (!strcmp(name, "block")) {
				F2FS_OPTION(sbi).discard_unit =
						DISCARD_UNIT_BLOCK;
						printk("discard unit = block");
			} else if (!strcmp(name, "segment")) {
				F2FS_OPTION(sbi).discard_unit =
						DISCARD_UNIT_SEGMENT;
				printk("discard unit = segment");
			} else if (!strcmp(name, "section")) {
				F2FS_OPTION(sbi).discard_unit =
						DISCARD_UNIT_SECTION;
							printk("discard unit = section");
			} else {
				kfree(name);
				return -EINVAL;
			}
		...
		case Opt_fdp_log_n:
			if (args->from && match_int(args, &arg)){
				printk("Opt_sungjin error");
				return -EINVAL;
			}
			// printk("opt sungjin!! %d",arg);
			unsigned int i;
			F2FS_OPTION(sbi).fdp_log_n=arg;
			// sbi->runtime_zone_reset=arg;
			// print_sungjin(F2FS_OPTION(sbi).fdp_log_n);
			sbi->FDP_PID_NR=F2FS_OPTION(sbi).fdp_log_n;
			// sbi->segs_per_type[CURSEG_WARM_DATA]=F2FS_OPTION(sbi).fdp_log_n;
			for(i=CURSEG_HOT_DATA;i<=CURSEG_COLD_NODE;i++){
				sbi->segs_per_type[i]=F2FS_OPTION(sbi).fdp_log_n;
			}
			// bioset_exit(&f2fs_bioset);
			// bioset_init(&f2fs_bioset, (NR_CURSEG_TYPE*sbi->FDP_PID_NR),
			// 		0, BIOSET_NEED_BVECS);
			// sbi->segs_per_type[CURSEG_WARM_DATA]=8;
			// sbi->segs_per_type[CURSEG_HOT_DATA]=8;
			// for(i=CURSEG_HOT_DATA;i<=CURSEG_COLD_DATA;i++){
			// 	sbi->segs_per_type[i]=F2FS_OPTION(sbi).fdp_log_n;
			// }
			printk("F2FS_OPTION(sbi).fdp_log_n %u",F2FS_OPTION(sbi).fdp_log_n);
			break;
...
try_onemore:
	...
	// sbi->FDP_PID_NR=raw_super->FDP_PID_NR;
	sbi->FDP_PID_NR=8;
	for(int i = 0; i <NO_CHECK_TYPE;i++){
		sbi->segs_per_type[i]=1;
	}
	sbi->segs_per_type[CURSEG_COLD_DATA_PINNED]=1;
	sbi->segs_per_type[CURSEG_ALL_DATA_ATGC]=1;
	sbi->segs_per_type[CURSEG_WARM_DATA]=(1);
	sbi->segs_per_type[CURSEG_COLD_DATA]=(1);
	sbi->segs_per_type[CURSEG_HOT_DATA]=(1);

	sbi->segs_per_type[CURSEG_WARM_NODE]=(1);
	sbi->segs_per_type[CURSEG_COLD_NODE]=(1);
	sbi->segs_per_type[CURSEG_HOT_NODE]=(1);
	...
	sbi->ino_pid_map = kvmalloc(sizeof(uint32_t)*(COFDP_PROTOTYPE_CUR_MAX_FILE_SUPPORT_N), GFP_KERNEL);
	for(err=0;err<1<<20;err++){
		sbi->ino_pid_map[err]=0;
	}
	printk("F2FS_RAW_SUPER(sbi)->feature & F2FS_FEATURE_LOST_FOUND %u\n",F2FS_RAW_SUPER(sbi)->feature & F2FS_FEATURE_LOST_FOUND);
	if(F2FS_RAW_SUPER(sbi)->feature & F2FS_FEATURE_LOST_FOUND){
		sbi->sungjin=1;
		// sbi->sungjin=0;
	}else{
		sbi->sungjin=0;
	...
	// sbi->total_bio_submit_n=0;
	// sbi->total_bio_submit_sectors=0;
	sbi->discarded_size=0;
	sbi->punch_hole=0;
	sbi->punch_hole_issued =0;
	sbi->total_read_bio_submit_n=0;
	sbi->total_read_bio_submit_sectors=0;
	for(err=0;err<16;err++){
		sbi->fdp_pid_nomerge_stat[err]=0;
		sbi->fdp_pid_merge_try_stat[err]=0;
		// sbi->submit_bio_spent[err]=0;
		// spin_lock_init(&sbi->fdp_pid_lock[err]);
		mutex_init(&sbi->fdp_pid_lock[err]);
		// sbi->total_bio_submit_n[err]=0;
		// sbi->total_bio_submit_sectors[err]=0;
	}
	for(err=0;err<MAX_PLIST_NUM;err++){
		sbi->discarded_created_n[err]=0;
	}
	int type,temp,hash;
	for(type=DATA;i<=NODE;type++){
		for(temp=HOT;temp<=COLD;temp++){
			for(hash=0;hash<sbi->segs_per_type[type*3+temp];hash++){
					sbi->total_bio_submit_sectors[type][temp][hash]=0;
					sbi->total_bio_submit_n[type][temp][hash]=0;
			}
		}
	}
	for(err=0;err<1<<20;err++){
