...
int f2fs_check_nid_range(struct f2fs_sb_info *sbi, nid_t nid)
{
	if (unlikely(nid < F2FS_ROOT_INO(sbi) || nid >= NM_I(sbi)->max_nid)) {
		...
		// f2fs_check_nid_range: out-of-range nid=0, run fsck to fix.
		f2fs_warn(sbi, "%s: out-of-range nid=%x, run fsck to fix.",
			  __func__, nid);
		printk("F2FS_ROOT_INO(sbi) %u",F2FS_ROOT_INO(sbi));
		// printk("")
...
static int init_node_manager(struct f2fs_sb_info *sbi)
{
	...
	printk("nat_segs %u",nat_segs);
	printk("nm_i->nat_blocks %u",nm_i->nat_blocks);
	printk("NAT_ENTRY_PER_BLOCK %u",NAT_ENTRY_PER_BLOCK);
	printk("nm_i->max_nid %lu",nm_i->max_nid);
