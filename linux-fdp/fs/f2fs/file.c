...
void f2fs_truncate_data_blocks_range(struct dnode_of_data *dn, int count)
{
	...
	// printk("f2fs_truncate_data_blocks_range %d\n",count);
	...
	for (; count > 0; count--, addr++, dn->ofs_in_node++, cluster_index++) {
		...
		// sbi->punch_hole_issued++;
		int ret =f2fs_invalidate_blocks(sbi, blkaddr);
...
static int f2fs_punch_hole(struct inode *inode, loff_t offset, loff_t len)
{
	...
	int ret;
	// printk("f2fs_punch_hole %lu %lu",offset,len);
				...
				loff_t offset, loff_t len)
{
	...
	if ((f2fs_compressed_file(inode) || f2fs_is_pinned_file(inode)) &&
		...
		printk("f2fs_fallocate 11\n");
		return -EOPNOTSUPP;
	}
	if (mode & ~(FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE |
			FALLOC_FL_COLLAPSE_RANGE | FALLOC_FL_ZERO_RANGE |
			FALLOC_FL_INSERT_RANGE)){
		printk("f2fs_fallocate 22\n");
		return -EOPNOTSUPP;
	}
	inode_lock(inode);
	...
	if (mode & FALLOC_FL_PUNCH_HOLE) {
		if (offset >= inode->i_size){
			// printk("out here? %s",__func__);
			goto out;
		}
		// F2FS_I_SB(inode)->punch_hole_issued+=(len/4096);
...
// static int walk_cb(struct f2fs_sb_info *sbi,struct dir_context *ctx, const char *name, int namelen,
//                    loff_t offset, u64 ino, unsigned int d_type)
// {
//     struct walker_ctx *wctx = container_of(ctx, struct walker_ctx, ctx);
//     struct dentry *dentry;
//     struct inode *inode;walk_cb
...
//     dentry = lookup_one_len(name, d_inode(wctx->parent_dentry), namelen);
//     if (IS_ERR(dentry))
...
// 	sbi->ino_pid_map[fio->ino]=
//     // Recurse if it's a directory
//     if (S_ISDIR(inode->i_mode)) {
//         struct walker_ctx new_ctx = {
//             .parent_inode = inode,
//             .parent_dentry = dentry,
//         };
//         iterate_dir(file_open_root(dentry, NULL, ""), &new_ctx.ctx);
//     }
...
// void walk_dir_recursive(struct f2fs_sb_info *sbi,struct inode *dir_inode,uint32_t bitmap)
// {
//     struct walker_ctx ctx;
//     struct file *filp;

//     filp = dentry_open(&dir_inode->i_sb->s_root->dentry, O_RDONLY, current_cred());
//     if (IS_ERR(filp)) {
//         printk(KERN_ERR "Failed to open directory inode\n");
//         return;
//     }

//     ctx.parent_inode = dir_inode;
//     ctx.parent_dentry = filp->f_path.dentry;
//     ctx.ctx.actor = walk_cb;
//     ctx.ctx.pos = 0;
// 	ctx.bitma

//     iterate_dir(filp, &ctx.ctx);

//     filp_close(filp, NULL);
// }

static int f2fs_ioc_flexible_rg(struct file *filp, unsigned long arg)
{
	...
	if(inode->i_op!=&f2fs_dir_inode_operations){
		printk("frg why f2fs_ioc_flexible_rg to normal file");
		return 0;
	}
	// if not directory, return.
	// if not start with pp, return.

	if (copy_from_user(&new_rg_group, (void __user *)arg,
			   sizeof(new_rg_group)))
		return -EFAULT;
	uint32_t i;
	uint32_t prev_rg_group= sbi->ino_pid_map[inode->i_ino];
	printk("frg : %u prev_rg_group %u",new_rg_group,prev_rg_group);
	printk("frg %p sbi->ino_pid_map %p",sbi,sbi->ino_pid_map);
	for(i=0;i<COFDP_PROTOTYPE_CUR_MAX_FILE_SUPPORT_N;i++){
		if(sbi->ino_pid_map[i]==prev_rg_group){
			// sbi->ino_pid_map[i]=(COFDP_RG_FLEXIBLE_BIT | new_rg_group);
			WRITE_ONCE(sbi->ino_pid_map[i],(COFDP_RG_FLEXIBLE_BIT | new_rg_group));
			printk("frg %u(%u) changed to %u\n",i,prev_rg_group,new_rg_group);
		}
	}

	sbi->ino_pid_map[inode->i_ino]=(COFDP_RG_FLEXIBLE_BIT | new_rg_group);
	inode->i_generation=(COFDP_RG_FLEXIBLE_BIT | new_rg_group);
	return 0;
}
		...
		pgoff_t off, block_t block, block_t len, u32 flags)
{
	...
	if (flags & F2FS_TRIM_FILE_DISCARD) {
		if (bdev_max_secure_erase_sectors(bdev))
			...
			printk("f2fs_secure_erase");
			F2FS_I_SB(inode)->discarded_size +=nr_sects;
...
#define F2FS_COFDP_FLEXIBLE_RG		_IOW(F2FS_IOCTL_MAGIC, 16, __u32)
...
static long __f2fs_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	// case F2FS_COFDP_HANDLE_UPDATE:
	// 	// not supported yet
	// 	return 0;
	// case F2FS_COFDP_EVENT_LOG:
	// 	// not supported yet
	// 	return 0;
	case F2FS_COFDP_FLEXIBLE_RG:
		// not supported yet
		return f2fs_ioc_flexible_rg(filp,arg);

	// case F2FS_IOC_SUNGJIN:
	// 	printk("F2FS_IOC_SUNGJIN %d\n",arg);
	// 	return 0;
	default:
		return -ENOTTY;
	}
