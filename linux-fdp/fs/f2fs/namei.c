...
// static int parse_ppdir(char *name, uint32_t *bitmap)
// {
//     char *p = name;
//     uint32_t map = 0;
// 	printk("parse_ppdir");
...
{
    if (!name || !bitmap) {
        printk("frg parse_ppdir: null input %p %p\n",name,bitmap);
        return 0;
    }
    ...
    printk("frg parse_ppdir: input = %s\n", name);

    while (*p) {
        if (p[0] != 'p' || p[1] != 'p') {
            printk("frg parse_ppdir: missing 'pp' at %s\n", p);
            return 0;
        }
        p += 2;

        if (!isdigit(*p)) {
            printk("frg parse_ppdir: expected digit after 'pp' at %s\n", p);
            return 0;
        }
        ...
        while (isdigit(*p)) {
            x = x * 10 + (*p - '0');
            if (x > 30) {
                printk("frg parse_ppdir: number too big: %u\n", x);
                return 0;
            }
            p++;
        }
    ...
    *bitmap = map;
    printk("frg parse_ppdir: bitmap = 0x%x\n", map);
    return 1;
}
						...
						const char *name)
{
	...
	// if(parse_ppdir(name,&(inode->i_generation))){
	// 	// printk(" i am ppdir %s!! %u",name,inode->i_generation);
	// 	inode->i_generation |= COFDP_RG_FLEXIBLE_BIT;
	// }else{
		
	// }
	// printk("mkdir %s\n",name);

	inode->i_generation = dir->i_generation;

	sbi->ino_pid_map[ino]=dir->i_generation;


	// printk("%s - ino %d, gen %d",name,ino,inode->i_generation);
	if (S_ISDIR(inode->i_mode))
		      ...
		      struct dentry *dentry, umode_t mode)
{
	...
	// dentry->d_name.name
	printk("frg f2fs_mkdir %s\n",dentry->d_name.name);
	if(parse_ppdir(dentry->d_name.name,&(inode->i_generation))){
		printk("frg i am ppdir %s!! %u",dentry->d_name.name,inode->i_generation);
		inode->i_generation |= COFDP_RG_FLEXIBLE_BIT;
