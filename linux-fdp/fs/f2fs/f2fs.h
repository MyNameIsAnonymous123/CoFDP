...
#define COFDP_PROTOTYPE_CUR_MAX_FILE_SUPPORT_N (1<<20)
#define COFDP_RG_FLEXIBLE_BIT (1U<<31)
#define IS_COFDP_RG_FLEXIBLE_BITMAP(bitmap) ((bitmap&(COFDP_RG_FLEXIBLE_BIT)))
...
#define DEF_MAX_DISCARD_REQUEST		10240	/* issue 8 discards per round */
#define DEF_MIN_DISCARD_ISSUE_TIME	1	/* 50 ms, if exists */
#define DEF_MID_DISCARD_ISSUE_TIME	1	/* 500 ms, if device busy */
...
struct f2fs_sb_info {
	...
	struct task_struct *f2fs_sungjin_stat;	/* discard thread */
	struct task_struct* f2fs_sungjin_discard_thread;
...
#endif
	unsigned int sungjin;
	uint32_t* ino_pid_map;
	uint64_t fdp_pid_merge_try_stat[16];
	uint64_t fdp_pid_nomerge_stat[16];
	uint64_t segs_per_type[NR_CURSEG_TYPE];

	uint64_t total_bio_submit_n[3][6][8];
	uint64_t total_bio_submit_sectors[3][6][8];
	uint64_t total_read_bio_submit_n;
	uint64_t total_read_bio_submit_sectors;
	uint64_t discarded_size;
	uint64_t discarded_created_n[MAX_PLIST_NUM];
	// spinlock_t fdp_pid_lock[16];
	struct mutex fdp_pid_lock[16];
	uint64_t punch_hole;
	uint64_t punch_hole_issued;
};
...
static inline int random_get_bit(struct f2fs_sb_info *sbi,uint32_t bitmap)
{
    // int n, i;
	uint32_t r,n,i;

    n = hweight32(bitmap); // 1로 설정된 비트 수
    if (n == 0)
        return -1;
	// dump_stack();
    get_random_bytes(&r, sizeof(r));
    r %= n;
	// printk("random_get_bit %d",r);
    for (i = 0; i < 32; i++) {
        if ((bitmap >> i) & 1) {
            if (r-- == 0)
                return i;
	...
	printk("random_get_bit erroor@@@@@@@@@ %u\n",bitmap);
	dump_stack();
    // return sbi->segs_per_type[0]; // 이론적으로 도달 불가
	return 2;
}

// static inline int random_get_bit(uint32_t bitmap)
// {
//     int i, count, r;

//     count = hweight32(bitmap);  // 1로 설정된 비트 개수
//     if (count == 0)
//         return -1;
...
void f2fs_submit_merged_ipu_write_fdp(struct f2fs_sb_info *sbi,
					struct bio **bio, struct page *page,int pid);
