		...
		enum nvme_opcode op)
{
	...
	// cmnd->directive.dspec=cpu_to_le16(dspec);
	// cmnd->rw.dsmgmt |= dspec<<16; 
	// *dsmgmt |= streamid << 16;
	cmnd->rw.dspec=cpu_to_le16(dspec);
	...
	// 	if(req->fdp_pid==0){
	// 		printk("@@@@@@@@@@@@@@ why? req->fdp_pid %u cmnd->rw.dspec %u",req->fdp_pid,cmnd->rw.dspec);
	// 		dump_stack();
	// 	}
	// 	if(cmnd->rw.dspec==0){
	// 		printk("!!!!!!!!!!!!!! why? req->fdp_pid %u cmnd->rw.dspec %u",req->fdp_pid,cmnd->rw.dspec);
	...
	// cmnd->common.cdw11=cpu_to_le32(req->fdp_pid);

	// cmnd->common.cdw13=cpu_to_le32(req->fdp_pid);
	// cmnd->directive.dspec=cpu_to_le16(req->fdp_pid);
	// printk("")
...
// [   37.298631] --req->fdp_pid 256 		 cmnd->common.cdw13 0 		cmnd->directive.dspec 256
// [   37.298642] cmnd->rw.slba 72057594038025226 		nvme_sect_to_lba(ns, blk_rq_pos(req) 97290 		cmnd->rw.length 0		(blk_rq_bytes(req) >> ns->lba_shift) - 1 0


// 		*/
// 		printk("\n");
// 		printk("44req->fdp_pid %d \
// 		 cmnd->common.cdw13 %d \
// 		cmnd->directive.dspec %u",
// 		req->fdp_pid,
// 		cmnd->common.cdw13,
// 		cmnd->directive.dspec);
// 		printk("cmnd->rw.dsmgmt %u",cmnd->rw.dsmgmt);
// 		printk("cmd->rw.dspec %u",cmnd->rw.dspec);


// 		printk("44cmnd->rw.slba %lu \
// 		nvme_sect_to_lba(ns, blk_rq_pos(req) %lu \
// 		cmnd->rw.length %d\
// 		(blk_rq_bytes(req) >> ns->lba_shift) - 1 %u",
// 		cmnd->rw.slba,
