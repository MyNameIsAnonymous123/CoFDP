...
PoolId MemoryAllocator::addPool(folly::StringPiece name,
  ...
  if (ensureProvisionable &&
      cachelib::Slab::kSize * poolAllocSizes.size() > size) {
        // size 15099494 cachelib::Slab::kSize 4194304 MemoryAllocator::kMaxClasses 128 poolAllocSizes.size() 15
    printf("size %lu cachelib::Slab::kSize %lu MemoryAllocator::kMaxClasses %lu poolAllocSizes.size() %lu\n",
    size,cachelib::Slab::kSize,MemoryAllocator::kMaxClasses,poolAllocSizes.size());
