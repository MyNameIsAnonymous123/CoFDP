...
RegionManager::RegionManager(uint32_t numRegions,
    ...
    numInMemBuffers_=30;
  printf("----------------------RegionManager pid %d // numInMemBuffers_ %u",placementHandle_,numInMemBuffers_);
  XLOGF(INFO, "{} regions, {} bytes each", numRegions_, regionSize_);
  // printf("{} regions, {} bytes each", numRegions_, regionSize_)
...
Region::FlushRes RegionManager::flushBuffer(const RegionId& rid) {
  ...
  //  printf("RegionManager::flushBuffer rid.index %lu physicalOffset(addr) %lu",rid.index(),physicalOffset(RelAddress{rid, 0}));
...
void RegionManager::doReclaim() {
  ...
  if (region.getNumItems() != 0) {
    ...
    if (buffer.size() != sizeToRead) {
      ...
      // printf("Eviction OK %d // %u\n",rid.index(),sizeToRead);
...
void RegionManager::doEviction(RegionId rid, BufferView buffer) const {
  ...
  // printf("RegionManager::doEviction eviction %u",rid.index());
  if (buffer.isNull()) {
