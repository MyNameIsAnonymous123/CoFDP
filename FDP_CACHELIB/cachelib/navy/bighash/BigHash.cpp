...
BigHash::BigHash(Config&& config, ValidConfigTag)
    : checkExpired_(std::move(config.checkExpired)),
      ...
      placementHandle_{device_.allocatePlacementHandle()} {
        // BigHash created: buckets: 2621440, bucket size: 4096, base offset: 10737418240
  printf("----------------------BigHash pid %d",placementHandle_);
  XLOGF(INFO,
...
bool BigHash::writeBucket(BucketId bid, Buffer buffer) {
  ...
  // printf("BigHash::writeBucket bid index %lu getBucketOffset(bid) %lu\n",bid.index(),getBucketOffset(bid));
