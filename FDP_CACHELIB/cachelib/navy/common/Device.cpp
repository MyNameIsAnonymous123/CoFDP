...
static constexpr size_t kIOTimeoutMs = 1000000;
...
//     Device::readInternal error here curOffset 288567296 readSize 4194304
// Device::readFaile here@@@@@@@@@@
// E0313 05:12:55.362334  7742 RegionManager.cpp:348] Failed to read region 510 during reclaim. Region size to read: 16773120, Actually read: 0
// Device::readInternal error here curOffset 439562240 readSize 4194304
// Device::readFaile here@@@@@@@@@@
// E0313 05:12:55.538431  7742 RegionManager.cpp:348] Failed to read region 263 during reclaim. Region size to read: 16774656, Actually read: 0
// Device::readInternal error here curOffset 2285056000 readSize 4194304
// Device::readFaile here@@@@@@@@@@
// E0313 05:12:55.612713  7742 RegionManager.cpp:348] Failed to read region 629 during reclaim. Region size to read: 16776704, Actually read: 0
// F0313 05:12:55.727311  7737 RegionManager.cpp:569] Check failed: addr.offset() + size <= region.getLastEntryEndOffset() (8647168 vs. 1515520)
// *** Aborted at 1741842775 (Unix time, try 'date -d @1741842775') ***
// *** Signal 6 (SIGABRT) (0x1e13) received by PID 7699 (pthread TID 0x7f05c2000700) (linux TID 7737) (maybe from PID 7699, UID 0) (code: -6), stack trace: ***
// Device::readInternal error here curOffset 506671104 readSize 4194304
      printf("Device::readInternal error here curOffset %u readSize %u\n"
      ,curOffset,readSize);
      readIOErrors_.inc();
      return false;
    }
  ...
  if (encryptor_) {
    ...
    auto res = encryptor_->decrypt(
        ...
        offset);
    if (!res) {
      printf("Device::readInternal error here 22222222\n");
...
Buffer Device::read(uint64_t offset, uint32_t size) {
  ...
  // if(readSize>>(15>>20)){
  //   printf("Device::read %u %u",offset,size);
  // }
  if (!result) {
    printf("Device::readFaile here@@@@@@@@@@\n");
    return Buffer{};
  }
  // if(readSize>>(15>>20)){
  //   printf("Device::read OK!!\n");
  // }
...
// E0307 14:59:34.385288  5250 Device.cpp:559] [sync] IO error: [req 0x7f995c043130] 
// idx 0 fd 4 op read offset 8352374784 size 4096 data 0x7f9904030000 resubmitted 0 len=0 errno=11 
// (Resource temporarily unavailable)
  ...
  if (!result) {
    ...
    // IOOp::done error len 0 size 1048576
    // IOOp::done error len 0 size 1048576
    // IOOp::done error len 0 size 1048576
    // IOOp::done error len 0 size 1048576
    // IOOp::done error len 0 size 1048576
    ...
    // IOOp::done error len 0 size 1048576
    // IOOp::done error len 0 size 1048576
    if(parent_.opType_ == READ){
      printf("IOOp::done error len %lu size %u offset_ %lu\n",len,size_,offset_);
    }

  }
  // else{
  //   if(parent_.opType_ == READ){
  //     printf("IOOp::done GOOOOOOD len %lu size %u offset_ %lu\n",len,size_,offset_);
  //   }
  // }
...
bool AsyncIoContext::submitIo(IOOp& op) {
  op.startTime_ = getSteadyClock();
  // printf("AsyncIoContext::submitIo\n");
...
std::unique_ptr<folly::AsyncBaseOp> AsyncIoContext::prepAsyncIo(IOOp& op) {
  if (fdpNvmeVec_.size() > 0) {
    ...
    // printf("why?? fdpNvmeVec_.size()==0@\n");
  }

  std::unique_ptr<folly::AsyncBaseOp> asyncOp;
  IOReq& req = op.parent_;
  if (useIoUring_) {
    //  printf("IIIIIIIIIIIIIouring@@@@@@@@@2 test\n");
#ifndef CACHELIB_IOURING_DISABLE
    // printf("IIIIIIIIIIIIIouring@@@@@@@@@2 DEFF\n");
...
#ifndef CACHELIB_IOURING_DISABLE
  if (fdpNvmeVec_.size() > 0) {
    ret= fdpNvmeVec_[kDefaultFdpIdx]->allocateFdpHandle();
    return ret;
  }
#endif
  return ret;
...
folly::File openCacheFile(const std::string& fileName,
  ...
  // sungjin
  printf("----------------------------OpenCacheFile fileName %s\n",fileName.c_str());
  // const int flags{O_RDWR | O_CREAT | O_DIRECT | (isExclusiveOwner ? O_EXCL : 0)};

  int flags= {O_RDWR | O_CREAT | (isExclusiveOwner ? O_EXCL : 0)};

  if(fileName == "/dev/nvme0n1"){ // fdp
