...
int FdpNvme::allocateFdpHandle() {
  ...
  printf("phndl %u nextPIDIdx_ %u maxPIDIdx_ %u\n",phndl,nextPIDIdx_,kDefaultPIDIdx);
...
void FdpNvme::initializeFDP(const std::string& bdevName) {
  ...
  // sungjin
  // E0307 13:11:46.776486 14713 Device.cpp:1219] NVMe FDP mode could not be enabled Failed to initialize FDP; nruhsd is 0, Errno: 0
// 
  ...
  for (uint16_t i = 0; i <= maxPIDIdx_; ++i) {
    ...
    printf("FdpNvme::initializeFDP %u\n",placementIDs_[i]);
  }

  XLOGF(INFO, "Creating NvmeFdp, fd: {}, Num_PID: {}, 1st PID: {}, Last PID: {}",
...
void FdpNvme::prepFdpUringCmdSqe(struct io_uring_sqe& sqe,
  ...
  // cmd->cdw13 = (dspec << 16);
  // cmd->dspec= cpu_to_le32(dspec);
  // cmd->dspec = (dspec<<16);
  cmd->dspec = dspec;
  cmd->dspec= htole16(dspec);
  // printf("cmd->dspec %u dspec %u\n",cmd->dspec,dspec);
  cmd->addr = (uint64_t)buf;
  cmd->data_len = size;

  cmd->nsid = nvmeData_.nsId();

  // struct nvme_cmd_rw* cmd = (struct nvme_cmd_rw*)&sqe.cmd;
  // if (cmd == nullptr) {
  //   throw std::invalid_argument("Uring cmd is NULL!");
  // }
  // memset(cmd, 0, sizeof(struct nvme_cmd_rw));
  // cmd->opcode = opcode;
...
void FdpNvme::prepWriteUringCmdSqe(
  ...
  // printf("prepWriteUringCmdSqe @@ pid %u\n",pid);
  // if(pid!=1){
  // printf("prepWriteUringCmdSqe @@ pid %u\n",pid);
  // }
