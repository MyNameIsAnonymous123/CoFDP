  ...
  if (status.ok()) {
    ...
    if (output_directory_) {
      io_s = output_directory_->FsyncWithDirOptions(
          IOOptions(), dbg,
          DirFsyncOptions(DirFsyncOptions::FsyncReason::kNewFileSynced,(compact_->compaction->output_level()+2)));
    }
...
Status CompactionJob::OpenCompactionOutputFile(SubcompactionState* sub_compact,
  ...
  // sungjin
  // uint64_t fno =  sub_compact->current_output().meta.fd.GetNumber();
  // uint64_t fno = outputs.GetMetaData()->fd.GetNumber();
  uint64_t fno= file_number;
  int level = sub_compact->compaction->output_level();
  if(cfd->ioptions().fs->fno_level_map_[fno]!=-1 ){
    printf("OpenCompactionOutputFile why?????????? %s\n",fname.c_str());
  }
  cfd->ioptions().fs->fno_level_map_[fno]=level;

  // writable_file->fno_=sub_compact->current_output()->meta.fd.GetNumber();
  // writable_file->level_=
