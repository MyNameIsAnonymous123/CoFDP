...
Status BuildTable(
  ...
  std::string fname = TableFileName(ioptions.cf_paths, meta->fd.GetNumber(),
                                    meta->fd.GetPathId());
  uint64_t fno = meta->fd.GetNumber();
  if(ioptions.fs->fno_level_map_[fno]!=-1 ){
    printf("BuildTable why?????????? %s level %d\n",
    fname.c_str(),
    ...
    if(std::equal(sst.rbegin(),sst.rend(),fname.rbegin())){
      ioptions.fs->fno_level_map_[fno]=tboptions.level_at_creation;
    }else{
      printf("BuildTable why 22222222222222222222222?@# %s level %d\n",
      fname.c_str(),
      tboptions.level_at_creation);
    }
