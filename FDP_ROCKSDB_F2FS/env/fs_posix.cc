...
class PosixFileSystem : public FileSystem {
 public:
  ...
  // std::atomic<uint64_t> file_size_dist_[1024];
  std::mutex file_path_map_lock;

    bool run_bg_reset_worker_=false;
  struct time_stat_fdp{
    int ctime;
    uint64_t put;
    uint64_t get;
    uint64_t scan;
  };
  std::vector<time_stat_fdp> time_stat_fdps_;
  std::unique_ptr<std::thread> bg_reset_worker_ =
      nullptr; 
  void BackgroundStatTimeLapse(){
    cur_time=0;
    uint64_t prev_ops=0;
    printf("--------------BackgroundStatTimeLapse------------------------\n");
    while(run_bg_reset_worker_){
      ...
      if(cur_time%10==0){
        ...
        time_stat_fdps_.push_back({cur_time,cur_ops,cur_get_ops,cur_scan_ops});
        printf("%d\t%lu\t%lu\t%lu\t%lu\t",
        cur_time,cur_ops,cur_get_ops,cur_scan_ops, (cur_ops+cur_get_ops+cur_scan_ops)-prev_ops);
        printf("\n");
        prev_ops=(cur_ops+cur_get_ops+cur_scan_ops);
        fflush(stdout);
      }
      ...
      printf("--------------bye BackgroundStatTimeLapse------------------------ bye\n");
  ...
  ~PosixFileSystem(){
    ...
    for(auto t: time_stat_fdps_){
        printf("%d\t%lu\t%lu\t%lu\t",
        t.ctime,t.put,t.get,t.scan);
        printf("\n");
    }
  ...
  uint64_t ExtractFno(const std::string& path) {
    std::string number;
    
    // Iterate from the end to find the last numeric sequence before ".sst"
    for (auto it = path.rbegin(); it != path.rend(); ++it) {
        if (std::isdigit(*it)) {
            number = *it + number;  // Prepend to maintain order
        } else if (!number.empty()) {
            break;  // Stop when non-digit character is found after numbers
        }
    }

    return number.empty() ? 0 : std::stoull(number);
  }

  
  std::string ParsePIDDir(std::string src, std::string pid_dir) {
    size_t last_slash = src.find_last_of('/');  // Find the last '/'
    if (last_slash == std::string::npos) return src;  // Return original if no slash found
      // std::lock_guard lg(file_path_map_lock);

    std::string path = src.substr(0, last_slash);  // Extract directory path
    std::string filename = src.substr(last_slash + 1);  // Extract filename
      {
    std::lock_guard lg(file_path_map_lock);

    file_path_map[src]=(path + "/" + pid_dir + "/" + filename);
    }
    return path + "/" + pid_dir + "/" + filename;  // Construct new path
}


  std::string ResolveFnameWithPID(std::string fname){
    uint64_t fno;
    std::string ret;
    ...
    if (file_path_map.find(fname) != file_path_map.end()) {
      return file_path_map[fname]; 
    }
    }

    if(ends_with(fname,".log")){
      // return db_ptr_->WALPIDPath(fname);
       pid_dir_path=pid_directory_[offset];
      return ParsePIDDir(fname,pid_dir_path);
    }if(ends_with(fname,".sst")){
      // if(db_ptr_->IsFnoPIDPathSet(fno,ret)){
      //   return ret;
      // }
      int level=0;
      // if(db_ptr_){
        // level = db_ptr_->GetFnoOrignialLevel(fno);
        
      // }
      fno=ExtractFno(fname);
      level=fno_level_map_[fno];
      
      if(level==-1|| level>10){
        // printf("ResolveFnameWithPID @@@ why ?? %s level %d\n",fname.c_str(),level);
      }
      // int max_pdir= pid_directory_.size()-1;
      // if(cur_time<5000){
      //   // max_pdir=0;
      //   pid_dir_path=pid_directory_[0];
      // }else if(cur_time<10000){
      //   // max_pdir=1;
      //   pid_dir_path=pid_directory_[(fno%2)];

      // }else if(cur_time<15000){
      //   // max_pdir=2;
      //   // if(level==0){
      //   //   pid_dir_path=pid_directory_[2];

      //   // }else{
      //   //   pid_dir_path=pid_directory_[1];
      //   // }
      //     pid_dir_path=pid_directory_[(fno%3)];

      // }
      // // else if(cur_time<4000){
      // //   if(level==0){
      // //     pid_dir_path=pid_directory_[3];
      ...
      // // }
      // else{
      //     pid_dir_path=pid_directory_[(fno%4)];

      // }


      if((size_t)(level+(1+offset))<pid_directory_.size()){
        pid_dir_path=pid_directory_[level+(1+offset)];
      }else{
        pid_dir_path=pid_directory_[(pid_directory_.size()-1)];
      }
      return ParsePIDDir(fname,pid_dir_path);
    }else{
  ...
  IOStatus NewSequentialFile(const std::string& fname,
    ...
    FILE* file = nullptr;
    FileSystem::TorFSStopWatch t1("Meta",this);
    ...
    // sungjin
    new_fname= ResolveFnameWithPID(fname);
    // printf("NewSequentialFile %s\n",new_fname.c_str());
    // fname=ResolveFnameWithPID(fname);
    do {
      IOSTATS_TIMER_GUARD(open_nanos);
      fd = open(new_fname.c_str(), flags, GetDBFileMode(allow_non_owner_access_));
    } while (fd < 0 && errno == EINTR);
    if (fd < 0) {
      return IOError("While opening a file for sequentially reading", new_fname,
                     errno);
    }
  ...
  IOStatus NewRandomAccessFile(const std::string& fname,
    ...
    if (fd < 0) {
      s = IOError("While open a file for random read", new_fname, errno);
      return s;
    }
    SetFD_CLOEXEC(fd, &options);
  ...
  virtual IOStatus OpenWritableFile(const std::string& fname,
    ...
    if (fd < 0) {
      s = IOError("While open a file for appending", new_fname, errno);
      return s;
    }
    SetFD_CLOEXEC(fd, &options);
  ...
  IOStatus ReuseWritableFile(const std::string& fname,
    ...
    IOStatus s;
    int fd = -1;
    std::string resolved_old_fname;
    std::string resolved_new_fname;
    int flags = 0;
    FileSystem::TorFSStopWatch t1("Meta",this);
    // Direct IO mode with O_DIRECT flag or F_NOCAHCE (MAC OSX)
    if (options.use_direct_writes && !options.use_mmap_writes) {
    ...
    flags = cloexec_flags(flags, &options);
    // sungjin
    resolved_old_fname= ResolveFnameWithPID(old_fname);
    resolved_new_fname= ResolveFnameWithPID(fname);
    // printf("ReuseWritableFile %s old_fname %s\n",resolved_new_fname.c_str(),resolved_old_fname.c_str());
    do {
      IOSTATS_TIMER_GUARD(open_nanos);
      fd = open(resolved_old_fname.c_str(), flags,
    ...
    SetFD_CLOEXEC(fd, &options);
    // rename into place
    if (rename(resolved_old_fname.c_str(), resolved_new_fname.c_str()) != 0) {
      s = IOError("while reopen file for write", fname, errno);
      close(fd);
      return s;
    }
  ...
  IOStatus NewRandomRWFile(const std::string& fname, const FileOptions& options,
    ...
    SetFD_CLOEXEC(fd, &options);
    result->reset(new PosixRandomRWFile(fname, fd, options,this));
    return IOStatus::OK();
  }
  ...
  IOStatus NewMemoryMappedFileBuffer(
    ...
    std::string new_fname;
    new_fname= ResolveFnameWithPID(fname);
    // printf("NewMemoryMappedFileBuffer %s\n",new_fname.c_str());
    while (fd < 0) {
      IOSTATS_TIMER_GUARD(open_nanos);
      fd = open(new_fname.c_str(), flags, 0644);
      if (fd < 0) {
        // Error while opening the file
        if (errno == EINTR) {
          continue;
  ...
  IOStatus NewDirectory(const std::string& name, const IOOptions& /*opts*/,
    ...
    if (fd < 0) {
      ...
      result->reset(new PosixDirectory(fd, name,this));
    }
    return IOStatus::OK();
  }
  ...
  IOStatus DeleteFile(const std::string& fname, const IOOptions& /*opts*/,
    ...
    // }
  // uint64_t fsize;
  // GetFileSize(fname,IOOptions(),&fsize,nullptr);
  // file_size_dist_[(fsize>>20)].fetch_add(1);
  FileSystem::TorFSStopWatch t1("Meta",this);
    std::string new_name = ResolveFnameWithPID(fname);
    if (unlink(new_name.c_str()) != 0) {
      result = IOError("while unlink() file", fname, errno);
    }
    {
      std::lock_guard lg(file_path_map_lock);
      file_path_map.erase(new_name);
    }
    return result;
  }
  ...
  IOStatus GetFileSize(const std::string& fname, const IOOptions& /*opts*/,
    ...
    struct stat sbuf;
  FileSystem::TorFSStopWatch t1("Meta",this);
    std::string new_fname= ResolveFnameWithPID(fname);
    if (stat(new_fname.c_str(), &sbuf) != 0) {
      *size = 0;
      return IOError("while stat a file for size", new_fname, errno);
    } else {
      *size = sbuf.st_size;
    }
    return IOStatus::OK();
  ...
  IOStatus RenameFile(const std::string& src, const std::string& target,
                      const IOOptions& /*opts*/,
                      IODebugContext* /*dbg*/) override {
    // While renaming a file to /home/femu/f2fs_fdp_mount/CURRENT: /home/femu/f2fs_fdp_mount/000005.dbtmp: No such file or directory
    FileSystem::TorFSStopWatch t1("Meta",this);
    std::string new_target= ResolveFnameWithPID(target);
    std::string new_src=ResolveFnameWithPID(src);
    if (rename(new_src.c_str(), new_target.c_str()) != 0) {
      return IOError("While renaming a file to " + new_target, new_src, errno);
    }
    return IOStatus::OK();
  }
  ...
  FileSystem::TorFSStopWatch t1("Meta",this);
    ...
    if (fd < 0) {
      result = IOError("while open a file for lock", new_fname, errno);
    } else if (LockOrUnlock(fd, true) == -1) {
      result = IOError("While lock file", new_fname, errno);
      close(fd);
    } else {
  ...
  Status RegisterDbPaths(const std::vector<std::string>& paths) override {
    // sungjin
    // printf("RegisterDbPaths %s\n",paths[0].c_str());
    // pid_t pid = getpid();

    // if(fork()==0){
      // std::string cache_path = "/proc";
      // exec();
    // }
    pid_directory_.clear();
    pid_directory_fd_.clear();
    ...
    if (dir) {
      ...
      while ((entry = readdir(dir)) != NULL) {
          ...
          // if(starts_with(entry->d_name,"p0")){
          //   continue;
          // }
          // if(starts_with(entry->d_name,"p1")){
          //   continue;
          // }
          if(starts_with(entry->d_name,"p")){
            // if(starts_with(entry->d_name,"p4")){
            //   break;
            // }
            pid_directory_.push_back(entry->d_name);
            // std::string _pid_dir_name  = std::string(paths[0]) +"/"+std::string(entry->d_name);
            // printf("%s starts with p\n", _pid_dir_name.c_str());
            
            // int pid_fd = open(_pid_dir_name.c_str(),cloexec_flags(0, nullptr));
            // pid_directory_fd_.push_back(pid_fd);
      
          }
      }
        // std::vector<std::string> pid_directory_;

        std::sort(pid_directory_.begin(),pid_directory_.end());

        for(auto pde:pid_directory_){
          std::string _pid_dir_name  = std::string(paths[0]) +"/"+std::string(pde.c_str());
...
PosixFileSystem::PosixFileSystem()
    : forceMmapOff_(false),
      ...
      fno_level_map_=(int*)malloc(sizeof(int)*(1<<20));
      // memset(fno_level_map_,-1,sizeof(fno_level_map_));
    ...
    if (bg_reset_worker_ == nullptr) {
      ...
      bg_reset_worker_.reset(
          new std::thread(&PosixFileSystem::BackgroundStatTimeLapse, this));
    }
#if defined(ROCKSDB_IOURING_PRESENT)
