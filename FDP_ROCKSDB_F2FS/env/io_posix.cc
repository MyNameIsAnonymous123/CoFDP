...
bool PosixWrite(int fd, const char* buf, size_t nbyte, FileSystem* fs_) {
  ...
  while (left != 0) {
    ...
    ssize_t done =0;
    {    
          FileSystem::TorFSStopWatch t1("PosixWrite",fs_);
...
bool PosixPositionedWrite(int fd, const char* buf, size_t nbyte, off_t offset, FileSystem* fs_) {
  const size_t kLimit1Gb = 1UL << 30;
  ...
  while (left != 0) {
    ...
    ssize_t done=0;
    {
      FileSystem::TorFSStopWatch t1("PosixPositionedWrite",fs_);
      done = pwrite(fd, src, bytes_to_write, offset);
    }
    if (done < 0) {
      if (errno == EINTR) {
        continue;
...
PosixSequentialFile::PosixSequentialFile(const std::string& fname, FILE* file,
                                         ...
                                         const EnvOptions& options,FileSystem* fs)
    : filename_(fname),
      file_(file),
      fd_(fd),
...
IOStatus PosixSequentialFile::PositionedRead(uint64_t offset, size_t n,
  ...
  while (left > 0) {
    {
    FileSystem::TorFSStopWatch t1("PosixSequentialFile::PositionedRead",fs_);
...
Status PosixHelper::GetQueueSysfsFileValueofDirectory(
  ...
  // printf("GetQueueSysfsFileValueofDirectory %s\n",directory.c_str());
...
size_t PosixHelper::GetQueueSysfsFileValueOfFd(
  ...
  FILE* fp;
  size_t value = 0;
  // printf("GetQueueSysfsFileValueOfFd %s\n",fname.c_str());
...
IOStatus PosixRandomAccessFile::Read(uint64_t offset, size_t n,
  ...
  while (left > 0) {
    {
    FileSystem::TorFSStopWatch t1("PosixRandomAccessFile::PositionedRead",fs_);
...
IOStatus PosixMmapFile::Fsync(const IOOptions& /*opts*/,
                              IODebugContext* /*dbg*/) {
  printf("PosixMmapFile::Fsync %s",filename_.c_str());
#ifdef HAVE_FULLFSYNC
  if (::fcntl(fd_, F_FULLFSYNC) < 0) {
...
IOStatus PosixWritableFile::Append(const Slice& data, const IOOptions& /*opts*/,
  ...
  if (!PosixWrite(fd_, src, nbytes,fs_)) {
...
IOStatus PosixWritableFile::PositionedAppend(const Slice& data, uint64_t offset,
  ...
  const char* src = data.data();
  size_t nbytes = data.size();
  if (!PosixPositionedWrite(fd_, src, nbytes, static_cast<off_t>(offset),fs_)) {
...
#else   // HAVE_FULLFSYNC
  // printf("PosixWritableFile::Sync  %s\n",filename_.c_str());
  // nosync
  // return IOStatus::OK();
  FileSystem::TorFSStopWatch t1("PosixWritableFile::Sync",fs_);
  if (fdatasync(fd_) < 0) {
...
IOStatus PosixWritableFile::Fsync(const IOOptions& /*opts*/,
                                  IODebugContext* /*dbg*/) {
    printf("PosixWritableFile::Fsync %s",filename_.c_str());
#ifdef HAVE_FULLFSYNC
  if (::fcntl(fd_, F_FULLFSYNC) < 0) {
    return IOError("while fcntl(F_FULLFSYNC)", filename_, errno);
  }
...
IOStatus PosixRandomRWFile::Write(uint64_t offset, const Slice& data,
  ...
  const char* src = data.data();
  size_t nbytes = data.size();
  if (!PosixPositionedWrite(fd_, src, nbytes, static_cast<off_t>(offset),nullptr)) {
...
IOStatus PosixRandomRWFile::Read(uint64_t offset, size_t n,
  ...
  while (left > 0) {
    ssize_t done=0;
    {
        FileSystem::TorFSStopWatch t1("PosixRandomRWFile::Read",fs_);

    done = pread(fd_, ptr, left, offset);
    }
    if (done < 0) {
...
// nosync
  // return IOStatus::OK();
// printf("PosixRandomRWFile::Sync %s",filename_.c_str());
 FileSystem::TorFSStopWatch t1("PosixRandomRWFile::Sync",fs_);
  if (fdatasync(fd_) < 0) {
...
IOStatus PosixRandomRWFile::Fsync(const IOOptions& /*opts*/,
                                  IODebugContext* /*dbg*/) {
  printf("PosixRandomRWFile::Fsync %s\n",filename_.c_str());
#ifdef HAVE_FULLFSYNC
  if (::fcntl(fd_, F_FULLFSYNC) < 0) {
...
#endif
PosixDirectory::PosixDirectory(int fd, const std::string& directory_name, FileSystem* fs)
    : fd_(fd), directory_name_(directory_name), fs_(fs){
  is_btrfs_ = false;
#ifdef OS_LINUX
  struct statfs buf;
  int ret = fstatfs(fd, &buf);
...
IOStatus PosixDirectory::FsyncWithDirOptions(
  ...
  IOStatus s = IOStatus::OK();
  // printf("FsyncWithDirOptions %s\n",directory_name_.c_str());
  // nosync
  // return s;
  // FileSystem::TorFSStopWatch t1("PosixDirectory::FsyncWithDirOptions",fs_);
#ifndef OS_AIX
  if (is_btrfs_) {
  ...
  // Mac OS.
  printf("HAVE FULLSYNC?\n");
  assert(!is_btrfs_);
  if (fd_ != -1 && ::fcntl(fd_, F_FULLFSYNC) < 0) {
    return IOError("while fcntl(F_FULLFSYNC)", "a directory", errno);
  }
#else   // HAVE_FULLFSYNC
    if(!fs_->pid_directory_fd_.size()){
        if (fd_ != -1 && fsync(fd_) == -1) {
            // printf("fsync to root\n");
            s = IOError("While fsync", "a directory1", errno);
        }
        return s;
    }
    ...
    // 2 l0
    // 3 l1
    switch(dir_fsync_options._type) {
        case 0:
            if (fd_ != -1 && fsync(fd_) == -1) {
              // printf("fsync to root\n");
              s = IOError("While fsync", "a directory2", errno);
            }
            break;
        // case 1: 
        //     break;
        default:
            int _fd = fs_->pid_directory_fd_[dir_fsync_options._type-1];
            if(_fd<=0){
              // printf("why?????? %d ??\n",dir_fsync_options._type-1);
              return s;
            }
            // printf("fsync to  %d\n",dir_fsync_options._type);
            // dir_fsync_options._type 4 fd 0
            
            if (_fd != -1 && fsync(_fd) == -1) {
              // printf("dir_fsync_options._type %d fd %d\n",dir_fsync_options._type,_fd);
              // s = IOError("While fsync", "a directory3", errno);
            }
            break;
    }
