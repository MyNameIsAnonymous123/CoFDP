...
// void XNvmeBackend::GetPidInfo(std::vector<uint64_t>& info){
//   info.clear();
...
int XNvmeBackend::Init() {
  ...
  fd_ = open(dev.c_str(),O_RDWR|O_DIRECT);
  if (!dev_) {
  ...
  xnvme_ctx->dev = dev_;
  // err=xnvme_nvm_mgmt_send(xnvme_ctx,nsid,NVME_IOMR_MO_RUH_STATUS,1,&ruh_status_,sizeof(nvme_fdp_ruh_status));
  err=xnvme_nvm_mgmt_recv(xnvme_ctx,nsid,NVME_IOMR_MO_RUH_STATUS,1,&ruh_status_,sizeof(nvme_fdp_ruh_status));
  // print
  if (err) {
  ...
  printf("ruh_status.nruhsd %d /// fr ratio %u copied page %u block erase %u\n",
    ruh_status_.nruhsd,ruh_status_.free_space_ratio
    ,ruh_status_.copied_page,ruh_status_.block_erased);
  if (err < 0) printf("Failed to drain xNVMe queue\n");
  PlacementID pid;
  for(int i=0;i<ruh_status_.nruhsd;i++){
        /*
          uint16_t pid;
        uint16_t ruhid;
        uint32_t earutr;
      ...
      pid.pid=0;
      printf("%d %d %d %ld\n",
        ruh_status_.ruhss[i].pid,
        ruh_status_.ruhss[i].ruhid,
        ruh_status_.ruhss[i].earutr,
        ruh_status_.ruhss[i].ruamw);
      // pid.pid=cpu_to_le16(ruh_status_.ruhss[i].pid);
      pid.pid=ruh_status_.ruhss[i].pid;
      max_ph_id_ = pid.ph > max_ph_id_ ?pid.ph :max_ph_id_;
      max_rg_id_ = pid.rg > max_rg_id_ ? pid.rg : max_rg_id_;
  }
  printf("max_ph_id_ %lu max_rg_id_ %lu\n",max_ph_id_,max_rg_id_);
  // xnvme_mutex_.Lock();
  xnvme_queues_.push(xqueue);
  // xnvme_mutex_.Unlock();
  return err;
...
int XNvmeBackend::SubmitXNvmeAsyncCmd(struct xnvme_queue* xqueue,
  ...
  while (remaining_size) {
    ...
    else if(dio == DIO_WRITE)
      opcode = XNVME_SPEC_NVM_OPC_WRITE;
    else{
      // ctx->dev->be.state->fd;
      opcode = XNVME_SPEC_NVM_OPC_FLUSH;
      //  fsync(xnvme_ctx->dev->be.state->fd);
      //  break;
    }
      // xnvme_ctx->dev
...
int XNvmeBackend::SubmitDeallocate(const DeviceGeometry& geo) {
  ...
  dsm_range->cattr = 0;
  dsm_range->slba = (uint64_t)(geo.offset_ >> geo_->ssw);
