...
#define SUNGJIN_MIN_IO_SIZE_SHIFT 14
...
class Backend {
 public:
  ...
  virtual void GetPidInfo(std::vector<uint64_t>& info) =0 ;
...
class IOInterface {
 public:
  ...
  int NvmeFlush(uint64_t offset, uint32_t size, const void* buf,
                        uint16_t placementID);
  ...
  inline void GetPidInfo(std::vector<uint64_t>& info){
    be_interface_->GetPidInfo(info);
  }
 private:
