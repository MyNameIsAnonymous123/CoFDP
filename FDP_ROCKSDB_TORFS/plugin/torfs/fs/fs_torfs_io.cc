...
IOStatus FDPSequentialFile::ReadOffset(uint64_t offset, size_t n, Slice* result,
  ...
  fs_tor_->cache_read_size_.fetch_add(n);
  if (offset >= (uint64_t)cache_pos) {
    // cache hit
    fs_tor_->cache_read_hit_size_.fetch_add(n);
    *read_len = n;
...
{   
    TorFS::TorFSStopWatch t1("IOInterface::Read",fs_tor_);
   err = fs_tor_->io_interface_->Read(read_align_off, read_align_size,
                                       rcache + read_actual_sum);
                                      
                                       }
    if (err) {
      printf("err %d read_align_off %lu read_align_size %lu read_actual_sum %lu rcache %p",
      err,read_align_off,read_align_size,read_actual_sum,rcache);
            ...
            : fs_tor_->block_size_ * (tmp_size / fs_tor_->block_size_ + 1);
{
  ...
  TorFS::TorFSStopWatch t1("IOInterface::Read",fs_tor_);
    ...
    if (err) {
      printf("error? %d read_align_off %lu read_align_size %lu readAlignSum %lu\n",err,read_align_off,read_align_size,readAlignSum);
      fs_tor_->io_interface_->FreeBuf(rcache);
      return IOStatus::IOError("Failed to read data from file: %s",
                               fdp_file_->name_.c_str());
    }
...
IOStatus FDPWritableFile::DataSync() {
  ...
  if (size % (1<<SUNGJIN_MIN_IO_SIZE_SHIFT) != 0) {
    newSize = (1<<SUNGJIN_MIN_IO_SIZE_SHIFT) * (size / (1<<SUNGJIN_MIN_IO_SIZE_SHIFT) + 1);
    padding = newSize - size;
  }
  ...
  IOStatus s;
  {
    TorFS::TorFSStopWatch t1("AllocCap",fs_tor_);
    s= fs_tor_->AllocCap(newSize, segs);
  }
  if (!s.ok()) return s;
  ...
  for (size_t i = 0; i < segs.size(); i++) {
    {
      TorFS::TorFSStopWatch t1("IOInterface::Write",fs_tor_);
    if(false){
      err = fs_tor_->io_interface_->NvmeFlush(segs[i].offset, segs[i].size,
...
{
  // TorFS::TorFSStopWatch t1("IOInterface::Fsync",fs_tor_);
  // fs_tor_->io_interface_->NvmeFlush(0, 4096,nullptr,0);
  }
{
  TorFS::TorFSStopWatch t1("FlushAppendMetaData",fs_tor_);
...
void FDPWritableFile::SetWriteLifeTimeHint(Env::WriteLifeTimeHint hint) {
  uint64_t stream_option=fs_tor_->GetTorFSOption(STREAM_OPTION);
  /*
  WAL= SHORT
  L0 = MEIDUM
  L1 = MEDIUM
  ...
  */
  ...
  case NOSTREAM:
    fdp_file_->pid_=0;
    // printf("@@@@@@@@@@@@@@@@@@@@@@@@@@ option 0 fdp_file_->pid_ %d\n",fdp_file_->pid_);
    break;
  case WRITE_LIFE_TIME_HINT:
    /*
    WAL = 0
    L0,L1 = 1
    L2 = 2
    ...
    */
    ...
    if(hint<Env::WLTH_SHORT){
      fdp_file_->pid_=0;
    }else{
      // fdp_file_->pid_ =
      // (hint - Env::WLTH_SHORT) >= MAX_PID ? (MAX_PID - 1) : (hint - Env::WLTH_SHORT)+1;
      fdp_file_->pid_=(hint - Env::WLTH_SHORT+1);
    }
    //  printf("@@@@@@@@@@@@@@@@@@@@@@@@@@ option 1 %s level %d fdp_file_->pid_ %d\n",fdp_file_->is_sst_ ? "is_sst_" : "",fdp_file_->level_,fdp_file_->pid_);
    break;
  case LEVEL_SEPARATION:
    ...
    if(hint<Env::WLTH_SHORT){ 
      ...
      // fdp_file_->pid_=
      //   fdp_file_->level_ +1 >= MAX_PID ? MAX_PID-1 : fdp_file_->level_ +1 ;
    }
    
    // fdp_file_->pid_ = 
    //   fdp_file_->level_ >= MAX_PID ? (MAX_PID - 1) : fdp_file_->level_;
    // printf("@@@@@@@@@@@@@@@@@@@@@@@@@@ option 2 %s level %d fdp_file_->pid_ %d\n",fdp_file_->is_sst_ ? "is_sst_" : "",fdp_file_->level_,fdp_file_->pid_);
    break;
  case PROPOSAL:
    break;
  case FDP_SSD:
    if(hint<Env::WLTH_SHORT){ 
      ...
      pid.rg=1;
        ...
        // fdp_file_->pid_=
        //   fdp_file_->level_ +2;
        pid.rg=fdp_file_->level_ +2 >= (int)(fs_tor_->pid_info_[1]) ? (int)(fs_tor_->pid_info_[1] ): fdp_file_->level_ +2;
      }

      // fdp_file_->pid_=
      //   fdp_file_->level_ +1 >= MAX_PID ? MAX_PID-1 : fdp_file_->level_ +1 ;
    }
    fdp_file_->pid_=pid.pid;
    printf("fdp pid :%d\n",fdp_file_->pid_);
    break;
  default:
    break;
