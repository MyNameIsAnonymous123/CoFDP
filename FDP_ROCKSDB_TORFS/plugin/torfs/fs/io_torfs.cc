...
std::shared_ptr<Backend> XNvmeFactory::CreateInterface(
  ...
  // print_sungjin(config->dev_type);
  printf("XNvmeFactory::CreateInterface :: %s\n ",config->dev_type.c_str());
...
int IOInterface::NvmeFlush(uint64_t offset, uint32_t size, const void* buf,
                       uint16_t placementID){
  return DoIO(offset, size, buf, placementID, DIO_FLUSH);
}
...
uint64_t IOInterface::GetNlba() { 
  printf("be_interface_->GetNlb() %lu\n",be_interface_->GetNlb());
  return be_interface_->GetNlb(); 
  }

uint64_t IOInterface::GetBlockSize() {
  printf("be_interface_->GetLbaShift() %u\n",be_interface_->GetLbaShift());
