...
#undef DeleteFile
  ...
  virtual void SetTorFSOption(std::vector<uint64_t>) {}
...
class FSWritableFile {
 public:
  ...
  FSWritableFile(const FileOptions& options)
      : last_preallocated_block_(0),
        preallocation_block_size_(0),
        io_priority_(Env::IO_TOTAL),
        write_hint_(Env::WLTH_NOT_SET),
  ...
  virtual void SungjinSetLevel(int){
...
class FileSystemWrapper : public FileSystem {
 public:
  ...
  void SetTorFSOption(std::vector<uint64_t> torfsoption) override{
    target_->SetTorFSOption(torfsoption);
  }
...
class FSWritableFileWrapper : public FSWritableFile {
 public:
  ...
  void SungjinSetLevel(int level){
    target_->SungjinSetLevel(level);
  }
