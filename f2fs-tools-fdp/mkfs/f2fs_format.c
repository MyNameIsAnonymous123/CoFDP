...
static void sungjin_fdp_make_pid_info(void){
	struct xnvme_opts opts = xnvme_opts_default();
	struct xnvme_dev* dev_ = NULL;
	const struct xnvme_geo* geo_ = NULL;
	struct xnvme_queue* queues_;
	int err;
    uint64_t nlb;
    uint64_t nsid;
	int i;

	dev_ = xnvme_dev_open(c.devices[0].path, &opts);
    if (!dev_) {
        printf("error xnvme_dev_open\n");
        return;
    }
	geo_ = xnvme_dev_get_geo(dev_);
	err=xnvme_queue_init(dev_,1,0,&queues_);
    if(err){
        printf("Error xnvme_queue_initd\n");
    	return;
    }

	struct xnvme_cmd_ctx* xnvme_ctx;
	xnvme_ctx = xnvme_queue_get_cmd_ctx(queues_);

    xnvme_ctx->async.cb = async_cb;
    xnvme_ctx->async.cb_arg = queues_;
    xnvme_ctx->dev = dev_;

    nlb=((1<<20)>>geo_->ssw)-1;
    nsid=xnvme_dev_get_nsid(dev_);
	int NVME_IOMR_MO_RUH_STATUS =1;
	int mos =1;
	...
	err = xnvme_queue_drain(queues_);
	if (err < 0) printf("Failed to drain xNVMe queue\n");

	c.FDP_PID_NR=ruh_status.nruhsd;
		// set_sb(FDP_PID_NR,c.FDP_PID_NR);
	printf("FDP npid : %d\n",c.FDP_PID_NR);
	for(i = 0;i<ruh_status.nruhsd;i++){
		// if(ruh_status.ruhss[i].pid==0){
		// 	continue;
		// }
		// if(ruh_status.ruhss[i].pid==1){
		// 	continue;
		// }
		c.FDP_PID_LIST[i]=ruh_status.ruhss[i].pid;
		c.FDP_PID_NID_LIST[i]=c.next_free_nid++;
		
		printf("FDP PID[%d] : %d (%d)\n",
		i,c.FDP_PID_LIST[i],c.FDP_PID_NID_LIST[i]);
	}
	


	xnvme_queue_term(queues_);
...
static int f2fs_prepare_super_block(void)
{
	...
	c.cur_seg=malloc(sizeof(uint32_t)*MAX_ACTIVE_NODE_LOGS*6);
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) {
		...
		unsigned int cs;
		c.cur_seg[0]=0;
			// for(cs =1 ; cs<c.max_open_zone/2;cs++){
		for(cs =1 ; cs<MAX_ACTIVE_NODE_LOGS*3;cs++){
			c.cur_seg[cs]=next_zone((cs-1));
				// printf("")
			// print_sungjin(c.cur_seg[cs]);
			printf("c.cur_seg[%u] %u\n",cs,c.cur_seg[cs]);
		}
	}	
	else {
		c.cur_seg[CURSEG_HOT_NODE] = 0;
...
static int f2fs_write_check_point_pack(void)
{
	...
	// set_cp(FDP_PID_NR,c.FDP_PID_NR);
	...
	if(PID_DIR_ENABLED){
		// PID 0 ---> SYSTEM
		// PID 1 ---> ROOT
		// PID 2 ---> FIRST DIRECTORY
		for(i=3;i<MAX_ACTIVE_DATA_LOGS;i++){
			set_cp(cur_data_segno[i],c.cur_seg[i+6]);
			printf("cur_data_segno[%d] c.cur_seg[%u] %u\n",i,i+6,c.cur_seg[i+6]);
		}

		// cur_data_segno[63] c.cur_seg[69] 0

		for(i=3;i<MAX_ACTIVE_NODE_LOGS;i++){
			set_cp(cur_node_segno[i],c.cur_seg[i+67]);
			printf("cur_node_segno[%d] c.cur_seg[%u] %u\n",i,i+67,c.cur_seg[i+67]);
		}
	}



	set_cp(cur_node_blkoff[0], 1 + c.quota_inum + c.lpf_inum + c.FDP_PID_NR);
	set_cp(cur_data_blkoff[0], 1 + c.quota_dnum + c.lpf_dnum + c.FDP_PID_NR);
	set_cp(valid_block_count, 2 + c.quota_inum + c.quota_dnum +
			c.lpf_inum + c.lpf_dnum + c.FDP_PID_NR*2);
	...
	set_cp(valid_node_count, 1 + c.quota_inum + c.lpf_inum + c.FDP_PID_NR);
	set_cp(valid_inode_count, 1 + c.quota_inum + c.lpf_inum + c.FDP_PID_NR);
	...
	*/
	...
	journal->n_nats = cpu_to_le16(1 + c.quota_inum + c.lpf_inum + c.FDP_PID_NR);
	...
	int k;
	for(k=0;k<c.FDP_PID_NR;k++){
		journal->nat_j.entries[i].nid = cpu_to_le32(c.FDP_PID_NID_LIST[k]);
		journal->nat_j.entries[i].ne.version = 0;
		journal->nat_j.entries[i].ne.ino = cpu_to_le32(c.FDP_PID_NID_LIST[k]);
		journal->nat_j.entries[i].ne.block_addr = cpu_to_le32(
				get_sb(main_blkaddr) +
				get_cp(cur_node_segno[0]) *
				c.blks_per_seg + i);
	...
	journal->sit_j.entries[0].se.vblocks =
				cpu_to_le16((CURSEG_HOT_NODE << 10) |
						(1 + c.quota_inum + c.lpf_inum + c.FDP_PID_NR));
	...
	printf("journal->sit_j.entries[0].se.vblocks %d vs %d\n",journal->sit_j.entries[0].se.vblocks,i);
	printf("%d , %d\n",journal->sit_j.entries[0].se.valid_map[0],journal->sit_j.entries[0].se.valid_map[1]);
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) {
		/* data sit for root */
		journal->sit_j.entries[1].segno = cp->cur_data_segno[0];
		journal->sit_j.entries[1].se.vblocks =
					cpu_to_le16((CURSEG_HOT_DATA << 10) |
							(1 + c.quota_dnum + c.lpf_dnum + c.FDP_PID_NR));
		...
		printf("journal->sit_j.entries[3].se.vblocks %d vs %d\n",journal->sit_j.entries[3].se.vblocks,i);
		printf("%d , %d\n",journal->sit_j.entries[3].se.valid_map[0],journal->sit_j.entries[3].se.valid_map[1]);
	...
	if (c.lpf_dnum) {
		...
		// sum_entry + 1 = 4
		printf("sum_entry + %d = %d\n",off,(sum_entry + off)->nid);
		(sum_entry + off)->ofs_in_node = 0;
		off++;
	}
	printf("FDP_PID_NR %u",c.FDP_PID_NR);
	for(k=0;k<c.FDP_PID_NR;k++){
		printf("k %d\n",k);
		(sum_entry+off)->nid = cpu_to_le32(c.FDP_PID_NID_LIST[k]);
		(sum_entry+off)->ofs_in_node=0;
		off++;
	}
	printf("okay %p \n",sum_compact);
	...
	if (c.lpf_inum) {
		...
		// sum->entries[1].nid 4
		printf("sum->entries[%d].nid %d",i,sum->entries[i].nid);
		sum->entries[i].ofs_in_node = 0;
	}

	for(k=0;k<c.FDP_PID_NR;k++){
		i++;
		sum->entries[i].nid=cpu_to_le32(c.FDP_PID_NID_LIST[k]);
		sum->entries[i].ofs_in_node=0;
	}
...
static int f2fs_discard_obsolete_dnode(void)
{
	...
	last_inode_pos = start_inode_pos +
		c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg + c.quota_inum + c.lpf_inum + c.FDP_PID_NR;
...
int dev_write_fdp_block(void* buf, __u64 blk_addr, unsigned int pid){
	struct xnvme_opts opts = xnvme_opts_default();
	struct xnvme_dev* dev_ = NULL;
	const struct xnvme_geo* geo_ = NULL;
	struct xnvme_queue* queues_;
    ...
    xnvme_prep_nvm(xnvme_ctx,XNVME_SPEC_NVM_OPC_WRITE,nsid,blk_addr,0);
    ...
    xnvme_ctx->cmd.nvm.cdw13.dspec = pid;
    
    // printf("%u / %u = %u\n",pid.rg,pid.ph,pid.pid);

    err = xnvme_cmd_pass(xnvme_ctx, buf, 4096, NULL, 0);
...
int write_fdp_inode(struct f2fs_node *inode, u64 blkaddr,unsigned int pid){
	if (c.feature & cpu_to_le32(F2FS_FEATURE_INODE_CHKSUM))
		inode->i.i_inode_checksum =
			cpu_to_le32(f2fs_inode_chksum(inode));
	return dev_write_fdp_block(inode, blkaddr,pid);
}

static block_t f2fs_add_default_dentry_PID(int index){
	...
	dent_blk->dentry[0].ino = cpu_to_le32(c.FDP_PID_NID_LIST[index]);
	...
	data_blk_offset += c.cur_seg[CURSEG_HOT_DATA] * c.blks_per_seg +
		1 + c.quota_dnum + 1 + index;

	// if (dev_write_block(dent_blk, data_blk_offset)) {
	if(dev_write_fdp_block(dent_blk,data_blk_offset,c.FDP_PID_LIST[index])){
...
static int sungjin_write_pid_inode(int index){
	struct f2fs_node *raw_node;
	...
	raw_node->footer.nid = cpu_to_le32(c.FDP_PID_NID_LIST[index]);
	...
	raw_node->footer.next_blkaddr = cpu_to_le32(
			get_sb(main_blkaddr) +
			c.cur_seg[CURSEG_HOT_NODE] * c.blks_per_seg +
			1 + c.quota_inum + 1  + index);


	raw_node->i.i_mode = cpu_to_le16(0x41FF); /* 0700 */
	raw_node->i.i_links = cpu_to_le32(2);
	raw_node->i.i_uid = cpu_to_le32(c.root_uid);
	raw_node->i.i_gid = cpu_to_le32(c.root_gid);
	...
	raw_node->i.i_generation = c.FDP_PID_LIST[index];
	...
	// printf()
    sprintf(namebuf, "p%d", c.FDP_PID_LIST[index]);
	// printf("%s\n",namebuf);
	raw_node->i.i_namelen = le32_to_cpu(strlen(namebuf));
	memcpy(raw_node->i.i_name, namebuf, strlen(namebuf));
	...
	data_blk_nor = f2fs_add_default_dentry_PID(index);
	if (data_blk_nor == 0) {
	...
	main_area_node_seg_blk_offset = get_sb(main_blkaddr);
	main_area_node_seg_blk_offset += c.cur_seg[CURSEG_HOT_NODE] *
		c.blks_per_seg + c.quota_inum + 2 + index;
	if (write_fdp_inode(raw_node, main_area_node_seg_blk_offset,c.FDP_PID_LIST[index]) < 0) {
		MSG(1, "\tError: While writing the raw_node to disk!!!\n");
		err = -1;
		goto exit;
...
static int f2fs_add_default_dentry_root(void)
{
	...
	for(index=0;index<c.FDP_PID_NR;index++){
		char namebuf[256];
		memset(namebuf,0,sizeof(namebuf));
		sprintf(namebuf, "p%d", c.FDP_PID_LIST[index]);

		int len=strlen(namebuf);
		f2fs_hash_t hash = f2fs_dentry_hash(0, 0, (unsigned char *)namebuf, len);
		
		dent_blk->dentry[4+index].hash_code = cpu_to_le32(hash);
		dent_blk->dentry[4+index].ino = cpu_to_le32(c.FDP_PID_NID_LIST[index]);
		dent_blk->dentry[4+index].name_len = cpu_to_le16(len);
		dent_blk->dentry[4+index].file_type = F2FS_FT_DIR;	
		memcpy(dent_blk->filename[4+index], namebuf, F2FS_SLOT_LEN);
		// memcpy(dent_blk->filename[3+index+1], &namebuf[F2FS_SLOT_LEN],
		// 		len - F2FS_SLOT_LEN);
		test_and_set_bit_le((4+index), dent_blk->dentry_bitmap);
	}
...
static int f2fs_create_root_dir(void)
{
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_LOST_FOUND)) {
		...
		for(int i =0 ;i<c.FDP_PID_NR;i++){
			sungjin_write_pid_inode(i);
		}
	}
